...
class Inventory(models.Model):
    ...
    group = models.ForeignKey(
        InventoryGroup, related_name="inventories", null=True,
        on_delete=models.SET_NULL
    )
...
class Invoice(models.Model):
    created_by = models.ForeignKey(
        CustomUser, null=True, related_name="invoice",
        on_delete=models.SET_NULL
    )
...
class InvoiceItem(models.Model):
    ...
    def save(self, *args, **kwargs):
        if self.item.remaining < self.quantity:
            raise Exception(f"item with code {self.item.code} does not have enough quantitiy")
